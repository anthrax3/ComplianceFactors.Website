            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty((string)ViewState["SearchResult"]))
                {
                   // employee_name = txtEmployeeName.Text;
                    //case_title = txtCaseTitle.Text;

                }
                else
                {
                   // employee_name = SecurityCenter.DecryptText(Request.QueryString["empname"]);
                   // case_title = SecurityCenter.DecryptText(Request.QueryString["casetitle"]);


                }
                DataTable dtCases = ComplianceBLL.SearchCaseByEmp(employee_name, case_title);
                DataTable newDt = dtCases.Clone();
                ...
                {
                    gvsearchDetails.DataSource = dtCases; 
                }
