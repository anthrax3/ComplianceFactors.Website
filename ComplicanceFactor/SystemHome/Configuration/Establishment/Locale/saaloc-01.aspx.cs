    ...
    public partial class saaloc_01 : BasePage
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                {
                    lblLocaleHeading.Text = LocalResources.GetLabel("app_establishment_information_text") + " (" + Request.QueryString["localeText"] + ")";
                }
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            ...
            {
                SystemEstablishment InsertLocale = new SystemEstablishment();
                InsertLocale.s_establishment_locale_name = txtName.Text;
                InsertLocale.s_establishment_locale_description = txtDescriptoin.Value;
                InsertLocale.s_locale_id_fk = Request.QueryString["localeid"];
                InsertLocale.s_establishment_system_id_fk = Request.QueryString["editestablishmentid"];
                
                try
                {
                    SystemEstablishmentBLL.InsertEstablishmentLocale(InsertLocale);
                }
                catch (Exception ex)
                {
                    //Log here
