    ...
    public partial class saaloc_01 : BasePage
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                {
                    lblLocaleHeading.Text = LocalResources.GetLabel("app_facility_information_text") + " (" + Request.QueryString["localeText"] + ")";
                }
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            ...
            {
                SystemFacility InsertLocale = new SystemFacility();
                InsertLocale.s_facility_locale_name = txtName.Text;
                InsertLocale.s_facility_locale_description = txtDescriptoin.Value;
                InsertLocale.s_locale_id_fk = Request.QueryString["localeid"];
                InsertLocale.s_facility_system_id_fk = Request.QueryString["editFacilityId"];
                
                try
                {
                    SystemFacilityBLL.InsertFacilityLocale(InsertLocale);
                }
                catch (Exception ex)
                {
                    //Log here
