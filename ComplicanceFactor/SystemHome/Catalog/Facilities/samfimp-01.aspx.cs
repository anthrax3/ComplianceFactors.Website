...
{
    public partial class samfimp_01 : BasePage
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                if (!IsPostBack)
                {
                    ...
                    //lblBreadCrumb.Text = "<a href=/SystemHome/sahp-01.aspx>" + LocalResources.GetGlobalLabel("app_nav_system") + "</a>&nbsp;" + " >&nbsp;" + LocalResources.GetGlobalLabel("app_manage_facilities_text");
                    ...
                    ddlFacilityType.SelectedValue = "app_ddl_all_text";
            ...
            catch (Exception ex)
            {
                ...
                if (ConfigurationWrapper.LogErrors == true)
                {
                    ...
                    {
                        Logger.WriteToErrorLog("samfimp-01.aspx", ex.Message, ex.InnerException.Message);
                    }
                    else
                    {
                        Logger.WriteToErrorLog("samfimp-01.aspx", ex.Message);
                    }
                }
            }
        }
        ...
        {
            Response.Redirect("~/SystemHome/Catalog/Facilities/saanfin-01.aspx");
        }
        /// <summary>
        /// Search Result
        /// </summary>
        ...
        {
            SystemFacility searchFacility = new SystemFacility();
            searchFacility.s_facility_id_pk = txtFacilityId.Text;
            searchFacility.s_facility_name = txtFacilityName.Text;
            searchFacility.s_facility_city = txtFacilityCity.Text;
            if (ddlStatus.SelectedValue == "app_ddl_all_text")
            {
                searchFacility.s_facility_status_id_fk = "0";
            }
            else
            {
                searchFacility.s_facility_status_id_fk = ddlStatus.SelectedValue;
            }
            if (ddlFacilityType.SelectedValue == "app_ddl_all_text")
            {
                searchFacility.s_facility_type_id_fk = "0";
            }
            else
            {
                searchFacility.s_facility_type_id_fk = ddlFacilityType.SelectedValue;
            }

            try
            {
                gvsearchDetails.DataSource = SystemFacilityBLL.SearchFacility(searchFacility);
        ...
        protected void gvsearchDetails_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            ...
            {
                Response.Redirect("~/SystemHome/Catalog/Facilities/saefin-01.aspx?edit=" + SecurityCenter.EncryptText(gvsearchDetails.DataKeys[rowIndex].Values[0].ToString()), false);
            }
            else if (e.CommandName.Equals("Copy"))
            {
                Response.Redirect("~/SystemHome/Catalog/Facilities/saanfin-01.aspx?copy=" + SecurityCenter.EncryptText(gvsearchDetails.DataKeys[rowIndex].Values[0].ToString()), false);
            }
            else if (e.CommandName.Equals("Archive"))
            {
                try
                {
                    SystemFacilityBLL.UpdateFacilityStatus(gvsearchDetails.DataKeys[rowIndex].Values[0].ToString());
                }
                catch (Exception ex)
                {
                    //TODO: Show user friendly error here
