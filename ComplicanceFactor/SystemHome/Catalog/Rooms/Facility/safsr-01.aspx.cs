...
{
    public partial class safsr_01 : BasePage
    {
        ...
        private void SearchResult()
        {
            try
            {
                ...
                {
                    catalog.c_session_facility_id_fk = txtFacilityId.Text;
                    catalog.c_session_facility_name = txtFacilityName.Text;

                }
                else
                {
                    catalog.c_session_facility_id_fk = SecurityCenter.DecryptText(Request.QueryString["id"]);
                    catalog.c_session_facility_name = SecurityCenter.DecryptText(Request.QueryString["name"]);


                }

                gvsearchDetails.DataSource = SystemCatalogBLL.FacilitySearch(catalog);
            ...
            catch (Exception ex)
            {
                ...
                if (ConfigurationWrapper.LogErrors == true)
                {
                    ...
                    {
                        Logger.WriteToErrorLog("safsr-01.aspx", ex.Message, ex.InnerException.Message);
                    }
                    else
                    {
                        Logger.WriteToErrorLog("safsr-01.aspx", ex.Message);
                    }
                }
            }
        ...
        protected void gvsearchDetails_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            ...
            {
                SessionWrapper.c_facility_name = row.Cells[0].Text + " (" + row.Cells[1].Text +")";
                SessionWrapper.c_facility_system_id_pk = gvsearchDetails.DataKeys[index]["c_facility_system_id_pk"].ToString();
