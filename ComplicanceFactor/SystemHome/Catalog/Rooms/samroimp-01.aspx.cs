...
{
    public partial class samroimp_01 : BasePage
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                if (!IsPostBack)
                {
                    ...
                    //lblBreadCrumb.Text = "<a href=/SystemHome/sahp-01.aspx>" + LocalResources.GetGlobalLabel("app_nav_system") + "</a>&nbsp;" + " >&nbsp;" + LocalResources.GetLabel("app_manage_rooms_text");

                    string navigationText;
                    Label lblBreadCrumb = (Label)Master.FindControl("lblBreadCrumb");
                    navigationText = BreadCrumb.GetCurrentBreadCrumb(SessionWrapper.navigationText);
                    hdNav_selected.Value = "#" + SessionWrapper.navigationText;
                    lblBreadCrumb.Text = navigationText + "&nbsp;" + " >&nbsp;" + "<a class=bread_text>" + LocalResources.GetLabel("app_manage_rooms_text") + "</a>";


                    //Bind Status
                    ddlStatus.DataSource = SystemRoomBLL.GetAllStatus(SessionWrapper.CultureName, "samroimp-01");
                    ddlStatus.DataBind();
                    ddlStatus.SelectedValue = "app_ddl_all_text";
                    // Bind RoomType
                    ddlRoomType.DataSource = SystemRoomBLL.GetRoomType(SessionWrapper.CultureName);
                    ddlRoomType.DataBind();
                    ddlRoomType.SelectedValue = "app_ddl_all_text";
            ...
            catch (Exception ex)
            {
                ...
                if (ConfigurationWrapper.LogErrors == true)
                {
                    ...
                    {
                        Logger.WriteToErrorLog("samroimp-01.aspx", ex.Message, ex.InnerException.Message);
                    }
                    else
                    {
                        Logger.WriteToErrorLog("samroimp-01.aspx", ex.Message);
                    }
                }
            }
        }
        ...
        {
            Response.Redirect("~/SystemHome/Catalog/Rooms/saanroin-01.aspx");
        }
        /// <summary>
        ...
        {
            SystemRoom searchRoom = new SystemRoom();

            searchRoom.s_room_id_pk = txtRoomId.Text;
            searchRoom.s_room_name = txtRoomName.Text;
            if (ddlStatus.SelectedValue == "app_ddl_all_text")
            {
                searchRoom.s_room_status_id_fk = "0";
            }
            else
            {
                searchRoom.s_room_status_id_fk = ddlStatus.SelectedValue;
            }
            if (ddlRoomType.SelectedValue == "app_ddl_all_text")
            {
                searchRoom.s_room_type_id_fk = "0";
            }
            else
            {
                searchRoom.s_room_type_id_fk = ddlRoomType.SelectedValue;
            }

           
            //searchRoom.c_room_facility = txtFacility.Text;
            try
            {
                gvsearchDetails.DataSource = SystemRoomBLL.SearchRoom(searchRoom);
        ...
        protected void gvsearchDetails_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            ...
            {
                Response.Redirect("~/SystemHome/Catalog/Rooms/saeroin-01.aspx?edit=" + SecurityCenter.EncryptText(gvsearchDetails.DataKeys[rowIndex].Values[0].ToString()), false);
            }
            else if (e.CommandName.Equals("Copy"))
            {
                Response.Redirect("~/SystemHome/Catalog/Rooms/saanroin-01.aspx?copy=" + SecurityCenter.EncryptText(gvsearchDetails.DataKeys[rowIndex].Values[0].ToString()), false);
            }
            else if (e.CommandName.Equals("Archive"))
            {
                try
                {
                    SystemRoomBLL.UpdateRoomStatus(gvsearchDetails.DataKeys[rowIndex].Values[0].ToString());
                }
                catch (Exception ex)
                {
                    //TODO: Show user friendly error here
