...
{
    public partial class p_secp_01 : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                PopulatePath(editCurriculumPath);
            }
        ...
        protected void dlSection_ItemDataBound(object sender, DataListItemEventArgs e)
        {
            ...
            {
                string c_curricula_path_section_id_fk = dlSection.DataKeys[e.Item.ItemIndex].ToString();
                if (string.IsNullOrEmpty(c_curricula_path_section_id_fk))
                {
                    e.Item.Style.Add("display", "none");
                }
                string c_curricula_path_section_enforce_seq_flag = DataBinder.Eval(e.Item.DataItem, "c_curricula_path_section_enforce_seq_flag").ToString();
                Literal ltlcheckSection = (Literal)e.Item.FindControl("ltlcheckSection");
                if (c_curricula_path_section_enforce_seq_flag == "True")
                {

                    ltlcheckSection.Text = "<input id=" + editCurriculumId + ',' + c_curricula_path_section_id_fk + " class='sectionrequired cursor_hand' type='checkbox' checked='checked'/>";
                }
                else
                {
        ...
        private void PopulatePath(string editCurriculumPathId)
        {
            SystemCurriculum path = new SystemCurriculum();
            path = SystemCurriculumBLL.GetCurriculumPath(string.Empty, editCurriculumPathId);
            txtPathName.Text = path.c_curricula_path_name;
            txtDescription.Value = path.c_curricula_path_Description;
            txtAbstract.Value = path.c_curricula_path_abstract;
            chkHeaderEnforceSectionsSequence.Checked = path.c_curricula_path_enforce_section_seq_flag;
            //txtComplete.Text = Convert.ToString(path.c_curricula_path_complete);
            //lblSectionCount.Text = Convert.ToString(path.c_curricula_path_sections);
            txtComplete.Text = Convert.ToString(dlSection.Items.Count);
            lblSectionCount.Text = Convert.ToString(dlSection.Items.Count);
            BindPathandSection();
        }
        public  void BindPathandSection()
        {
            //Get all section 
            GetPathSection();
            ...
            for (int i = 0; i < dlSection.Items.Count; i++)
            {
                ...
                if (!string.IsNullOrEmpty(c_curricula_path_course_seq_number))
                {
                    ...
                    //txtCourseComplete.Text = dsGetPath.Tables[1].Rows[i]["c_curricula_path_section_complete"].ToString();
                    //lblCourse.Text = dsGetPath.Tables[1].Rows[i]["c_curricula_path_section_courses"].ToString();
        ...
        private void BindPathCourse(GridView GridView, string c_curricula_path_section_id_pk)
        {
            ...
            {   // Update section complete and course
                //SystemCurriculumBLL.UpdateSectionCompleteAndCourse(c_curricula_path_section_id_pk,editCurriculumId);
        ...
        protected void gvCourse_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            ...
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                ...
                else
                {
                    ...
                    ltlcheck.Text = "<input id=" + c_curricula_path_course_id_fk + ',' + c_curricula_path_section_id_fk + " class='cousenotrequired cursor_hand' type='checkbox' />";
                }
        ...
        protected void gvCourse_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            if (e.CommandName.Equals("Remove"))
            {
                ...
                SystemCurriculumBLL.DeleteSinglePathcourse(c_curricula_path_course_id_fk, c_curricula_path_section_id_fk);
            ...
            else if (e.CommandName.Equals("Up"))
            {
                ...
                string c_current_curricula_path_course_id_fk = GridView1.DataKeys[gvRowIndex][0].ToString();//Current Course id
                string c_previous_curricula_path_course_id_fk = GridView1.DataKeys[gvRowIndex-1][0].ToString();//Previous course id
                ...
                SystemCurriculumBLL.UpdateCurriculaPathCourseSeqNumberforUp(c_current_curricula_path_course_id_fk,c_previous_curricula_path_course_id_fk, current_course_system_id_pk, previous_course_system_id_pk, c_curricula_path_section_id_fk);
             }
            else if (e.CommandName.Equals("Down"))
            {
                ...
                string c_current_curricula_path_course_id_fk = GridView1.DataKeys[gvRowIndex][0].ToString();//Current Course id
                string c_next_curricula_path_course_id_fk = GridView1.DataKeys[gvRowIndex + 1][0].ToString();//Next course id
                string current_course_system_id_pk = GridView1.DataKeys[gvRowIndex][2].ToString();//current path system id
                string next_course_system_id_pk = GridView1.DataKeys[gvRowIndex+1][2].ToString();//Next id
                SystemCurriculumBLL.UpdateCurriculaPathCourseSeqNumberforDown(c_current_curricula_path_course_id_fk, c_next_curricula_path_course_id_fk, current_course_system_id_pk, next_course_system_id_pk, c_curricula_path_section_id_fk);
            }
            BindPathandSection();
        }
           ...
           try
            {
                ...
                SystemCurriculum path = new SystemCurriculum();
                path.c_curricula_path_system_id_pk = editCurriculumPath;
                ...
                int result = SystemCurriculumBLL.UpdateCurriculaPath(path);
                //Close fancybox
                if (result != -1)
                {
        ...
        /// <summary>
        /// Update checked required field
        /// </summary>
        [System.Web.Services.WebMethod]
        public static void CourseRequired(string args, string args1, string args2)
        {
            string c_curricula_path_section_id_fk = args1.Trim();
            string c_curricula_path_course_id_fk = args.Trim();
            SystemCurriculumBLL.UpdateCheckedCourseRequiredField(c_curricula_path_section_id_fk, c_curricula_path_course_id_fk);
           
        }
        /// <summary>
        /// Update Unchecked required field
        /// </summary>
        [System.Web.Services.WebMethod]
        public static void CourseNotRequired(string args, string args1, string args2)
        {
            ...
            string c_curricula_path_section_id_fk = args1.Trim();
            string c_curricula_path_course_id_fk = args.Trim();
            SystemCurriculumBLL.UpdateUnCheckedCourseRequiredField(c_curricula_path_section_id_fk, c_curricula_path_course_id_fk);
        ...
        {
            string c_curricula_id_fk = args.Trim();
            string c_curricula_path_section_id_fk = args1.Trim();
            SystemCurriculumBLL.UpdateCheckedEnforceCourse(c_curricula_id_fk, c_curricula_path_section_id_fk);    
        }
        /// <summary>
        ...
        public static void sectionnotrequired(string args, string args1)
        {
            string c_curricula_id_fk = args.Trim();
            string c_curricula_path_section_id_fk = args1.Trim();
            SystemCurriculumBLL.UpdateUnCheckedEnforceCourse(c_curricula_id_fk, c_curricula_path_section_id_fk);
