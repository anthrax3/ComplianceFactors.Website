...
namespace ComplicanceFactor.SystemHome.Catalog.Locations.Locale
{
    public partial class savloc_01 : BasePage
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            else if (!string.IsNullOrEmpty(Request.QueryString["mode"]) && Request.QueryString["mode"] == "edit" && !string.IsNullOrEmpty(Request.QueryString["locationid"]))
            {
                SessionWrapper.Locale.Clear();
                dtLocale = SystemLocationBLL.GetLocationLocale(Request.QueryString["locationid"]);
        ...
        public static void DeleteLocale(string args, string args1)
        {
            try
            {
                ...
                SystemLocationBLL.DeleteLocationLocale(string.Empty, string.Empty, args.Trim());
